...
function VisitorPanel({ products, settings, setProducts, hasMore, isLoadingMore, loadMoreProducts, apiCategories, setOrders, publicShops = [], initialShopId }: { products: Product[], settings: Record<string, string>, setProducts: React.Dispatch<React.SetStateAction<Product[]>>, hasMore?: boolean, isLoadingMore?: boolean, loadMoreProducts?: () => void, apiCategories: any[], setOrders: React.Dispatch<React.SetStateAction<any[]>>, publicShops?: any[], initialShopId?: string }) {
  ...
  const categories = [
    ...
    const activeShopContext = selectedShopId 
    ? publicShops.find(s => s.id === selectedShopId) 
    : publicShops.find(s => s.id === 'main-shop') || publicShops[0];
        ...
        {currentTab === 'account' && (
           <div className="space-y-6">
             ...
             <div className="premium-card overflow-hidden mt-4">
                <div className="p-4 border-b border-gray-50 flex flex-col bg-gold-50/50">
                   <h3 className="font-bold text-primary">Store Contact Info</h3>
                   {activeShopContext?.name && <p className="text-xs text-neutral-500 font-bold mt-1">{activeShopContext.name}</p>}
                </div>
                
                {activeShopContext?.address && (
                  <div className="p-4 border-b border-gray-50 flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-neutral-600 whitespace-pre-wrap leading-relaxed">{activeShopContext.address}</p>
                    </div>
                  </div>
                )}
                
                {(activeShopContext?.openingTime || activeShopContext?.closingTime) && (
                  <div className="p-4 border-b border-gray-50 flex items-center gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-neutral-600">
                        {activeShopContext.openingTime || '09:00 AM'} - {activeShopContext.closingTime || '09:00 PM'}
                      </p>
                    </div>
                  </div>
                )}
                ...
                {(activeShopContext?.whatsapp || activeShopContext?.phone || settings.admin_phone) && (
                  <a href={`https://wa.me/${(activeShopContext?.whatsapp || activeShopContext?.phone || settings.admin_phone).replace(/\D/g, '')}`} target="_blank" rel="noreferrer" className="p-4 border-b border-gray-50 flex items-center gap-4 hover:bg-gold-50 transition-colors">
                     ...
                     <div className="flex-1">
                       ...
                       <p className="text-xs text-neutral-500">{activeShopContext?.whatsapp || activeShopContext?.phone || settings.admin_phone}</p>
                     </div>
                  </a>
                )}
                
                {(activeShopContext?.phone) && (
                  <a href={`tel:${activeShopContext.phone}`} className="p-4 border-b border-gray-50 flex items-center gap-4 hover:bg-gold-50 transition-colors">
                     ...
                     <div className="flex-1">
                       ...
                       <p className="text-xs text-neutral-500">{activeShopContext.phone}</p>
                     </div>
                  </a>
                )}
                
                {(activeShopContext?.email || settings.admin_email) && (
                  <a href={`mailto:${activeShopContext?.email || settings.admin_email}`} className="p-4 border-b border-gray-50 flex items-center gap-4 hover:bg-gold-50 transition-colors">
                     ...
                     <div className="flex-1">
                       ...
                       <p className="text-xs text-neutral-500">{activeShopContext?.email || settings.admin_email}</p>
                     </div>
                  </a>
                )}
